...
import { UpdateDesignerToolSchema, UpdateDesignerToolType } from '@/schema/designer-tool.schema';
...
const UiToolAction: FC<ToolActionProps> = ({tool}) => {
...
const router = useRouter()
  ...
  const {mutate: removeTool} = useMutation({
    mutationFn: () => ( axios.delete(`/api/designer-tool/edit/${tool.id}`)),
    onSuccess: () => {
    ...
    onSettled: () => {
      router.push("/admin/ui-tools")
      queryClient.invalidateQueries(["designertools"])
    }
  })

  const {register,handleSubmit,formState:{errors},reset} = useForm<UpdateDesignerToolType>({
    resolver: zodResolver(UpdateDesignerToolSchema),
    defaultValues: {
      name: tool.name,
      path: tool.path,
  ...
  const {mutate: editTool} = useMutation({
    mutationFn: (data: UpdateDesignerToolType) => ( axios.post(`/api/designer-tool/edit/${tool.id}`, data)),
    onSuccess: () => {
 ...
 function onSubmit(data: UpdateDesignerToolType) {
  setLoading(true)
  editTool(data)
 }
